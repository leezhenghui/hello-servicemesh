			...
			driver = "raw_exec"
				config {
					command = "/opt/consul/bin/consul"
						...
						"connect", "proxy",
						"-service", "add-svc",
						"-service-addr", "${NOMAD_ADDR_addsvcTask_http}",
						"-listen", ":${NOMAD_PORT_tcp}",
						"-register",
						"-register-id", "${NOMAD_PORT_tcp}",
						]

					# a quick approach to verify the value of interpolated variable
					# command = "/bin/bash"
					# args    = [
					# "-c", "echo ${NOMAD_ADDR_addsvcTask_http}, :${NOMAD_PORT_tcp}",
					# ]
				}
