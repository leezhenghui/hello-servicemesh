  ...
  group "frontendGroup" {
    ...
    task "frontendTask" {
      ...
      driver = "raw_exec"
			...
			env {
				ADD_SVC_ADDR = "http://localhost:${NOMAD_PORT_addsvcProxy_tcp}"
				SUB_SVC_ADDR = "http://localhost:${NOMAD_PORT_subsvcProxy_tcp}"
			}
			...
			driver = "raw_exec"
				config {
					command = "/opt/consul/bin/consul"
						...
						"connect", "proxy",
						"-service", "frontend",
						"-upstream", "add-svc:${NOMAD_PORT_tcp}",
						]
				}
			...
			driver = "raw_exec"
				config {
					command = "/opt/consul/bin/consul"
						...
						"connect", "proxy",
						"-service", "frontend",
						"-upstream", "sub-svc:${NOMAD_PORT_tcp}",
						]
				}
